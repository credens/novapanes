...
app.post('/api/contact', async (req, res) => { const { nombre, email, telefono, mensaje } = req.body; if (!nombre || !telefono) { return res.status(400).json({ success: false, message: 'El nombre y el teléfono son obligatorios.' }); } const transporter = nodemailer.createTransport({ service: 'gmail', auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_APP_PASSWORD, }, }); const emailBody = `<h1>📬 Nueva Consulta desde el Sitio Web 📬</h1><h3>Detalles del Contacto:</h3><ul><li><strong>Nombre:</strong> ${nombre}</li><li><strong>Teléfono:</strong> ${telefono}</li>${email ? `<li><strong>Email:</strong> ${email}</li>` : ''}</ul><h3>Mensaje:</h3><p style="background-color:#f4f4f4; padding: 15px; border-radius: 5px;">${mensaje || 'No se ha escrito ningún mensaje.'}</p>`; const mailOptions = { from: `"NOVA Panes Web" <${process.env.EMAIL_USER}>`, to: 'panes.nova@gmail.com', subject: `Nueva consulta de: ${nombre}`, html: emailBody, }; try { await transporter.sendMail(mailOptions); res.status(200).json({ success: true, message: '¡Gracias por tu mensaje! Nos pondremos en contacto contigo pronto.' }); } catch (error) { console.error("Error al enviar email de contacto:", error); res.status(500).json({ success: false, message: 'Hubo un error al enviar el mensaje. Por favor, intenta de nuevo.' }); }});
app.post('/api/submit-order', async (req, res) => { const orderData = req.body; try { const orders = readJsonFile(ORDERS_FILE_PATH); const newOrder = { id: `order_${Date.now()}`, customer: orderData.customer, items: orderData.items, total: orderData.total, paymentMethod: orderData.metodoPago, date: new Date().toISOString(), status: 'pending' }; orders.unshift(newOrder); writeJsonFile(ORDERS_FILE_PATH, orders); } catch (error) { console.error("Error al guardar el pedido:", error); } const transporter = nodemailer.createTransport({ service: 'gmail', auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_APP_PASSWORD } }); const emailBody = `<h1>🍞 Pedido Recibido</h1><h2>Cliente: ${orderData.customer.nombre}</h2><ul><li>Email: ${orderData.customer.email}</li><li>Teléfono: ${orderData.customer.telefono}</li><li>Dirección: ${orderData.customer.direccion}, ${orderData.customer.ciudad}</li></ul><h2>Pedido:</h2><table border="1" cellpadding="5" cellspacing="0" style="width:100%; border-collapse:collapse;"><thead><tr style="background-color:#f2f2f2;"><th>Producto</th><th>Cant.</th><th>Subtotal</th></tr></thead><tbody>${orderData.items.map(i => `<tr><td>${i.name}</td><td style="text-align:center;">${i.quantity}</td><td style="text-align:right;">$${(i.price * i.quantity).toLocaleString('es-AR')}</td></tr>`).join('')}</tbody></table><h3 style="text-align:right;">Total: $${orderData.total.toLocaleString('es-AR')}</h3><p><strong>Pago:</strong> ${orderData.metodoPago}</p>`; const mailOptions = { from: `"NOVA Panes Web" <${process.env.EMAIL_USER}>`, to: 'panes.nova@gmail.com', subject: `Pedido de ${orderData.customer.nombre}`, html: emailBody }; try { await transporter.sendMail(mailOptions); res.status(200).json({ message: 'OK' }); } catch (e) { console.error("Error al enviar email:", e); res.status(500).json({ message: 'Error en email.' }); } });
...
try {
    app.listen(port, () => {
        ...
        if (err.code === 'EADDRINUSE') { console.error(`\n🚨 Error: El puerto ${port} ya está en uso. 🚨\n   -> Para solucionarlo, busca y detén el proceso con: lsof -i :3000 y luego kill -9 <PID>`); } else { console.error('Error al iniciar el servidor:', err); }
        process.exit(1);
    });
} catch (error) {
