...
app.post('/api/admin/verify', express.json(), async (req, res) => {
    ...
    try {
        ...
        if (match) { console.log("✅ ¡COINCIDEN!"); } else { console.log("❌ ¡NO COINCIDEN!"); }
        console.log("=============================================\n\n");
...
try {
    app.listen(port, () => {
        ...
        if (err.code === 'EADDRINUSE') {
            ...
            console.error('   -> Asegúrate de que no haya otro servidor corriendo en la misma terminal.');
            console.error('   -> Para solucionarlo, busca y detén el proceso con: lsof -i :3000 y luego kill -9 <PID>');
        } else {
