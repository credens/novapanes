// ===================================================
//      ARCHIVO server.js (CON GESTIÓN DE PEDIDOS)
// ===================================================
...
function readJsonFile(filePath) {
    try {
        if (!fs.existsSync(filePath)) {
            if (filePath.endsWith('orders.json') || filePath.endsWith('products.json') || filePath.endsWith('categories.json')) {
                fs.writeFileSync(filePath, '[]'); // Crea el archivo si no existe
                return [];
            }
        }
        return JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    } catch (error) {
        throw new Error(`No se pudo leer o crear el archivo: ${path.basename(filePath)}`);
    }
}
...
app.post('/api/submit-order', async (req, res) => {
    ...
    try {
        ...
        const newOrder = { id: `order_${Date.now()}`, customer: orderData.customer, items: orderData.items, total: orderData.total, paymentMethod: orderData.metodoPago, date: new Date().toISOString(), status: 'pending' };
    ...
    const mailOptions = { from: `"NOVA Panes Web" <${process.env.EMAIL_USER}>`, to: 'panes.nova@gmail.com', subject: `Pedido de ${orderData.customer.nombre}`, html: emailBody };
    try { await transporter.sendMail(mailOptions); res.status(200).json({ message: 'OK' }); } catch (e) { console.error("Error al enviar email:", e); res.status(500).json({ message: 'Error en email.' }); }
});
