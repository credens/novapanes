document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addToCart = (event, id) => { const p = products.find(prod => prod.id === id); if (!p) return; const q = parseInt(document.getElementById(`qty-${id}`).value); const item = cart.find(i => i.id === id); if (item) { if (item.quantity + q <= p.stock) item.quantity += q; else return alert(`Stock insuficiente`); } else { if (q > p.stock) return alert(`Stock insuficiente`); cart.push({ ...p, quantity: q }); } updateCartDisplay(); const btn = event.target; btn.textContent = '¡Agregado!'; btn.style.background = '#4CAF50'; setTimeout(() => { btn.textContent = 'Agregar al Carrito'; btn.style.background = '#B5651D'; }, 1000); };
