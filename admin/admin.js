document.addEventListener('DOMContentLoaded', () => {
    ...
    async function verifyPassword(password) {
        try {
            const response = await fetch(`${API_BASE_URL}/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) });
            const result = await response.json();
            if (result.success) { setPassword(password); loadInitialData(); } else { alert('Contraseña incorrecta.'); window.location.href = '/'; }
        } catch (error) { alert('Error al verificar la contraseña.'); window.location.href = '/'; }
    }
    ...
    function renderOrdersTable() {
        ...
        if (allOrders.length === 0) { ordersTableBody.innerHTML = '<tr><td colspan="5">No hay pedidos registrados.</td></tr>'; return; }
    ...
    categoriesList.addEventListener('click', async e => { if (e.target.classList.contains('btn-delete-category')) { const id = e.target.dataset.id; if (!confirm(`¿Eliminar la categoría "${id}"?`)) return; try { const response = await fetch(`${API_BASE_URL}/categories/${id}`, { method: 'DELETE', headers: { 'Authorization': getPassword() } }); if (!response.ok) throw new Error((await response.json()).message); loadInitialData(); } catch (error) { alert(error.message); } } });
    ordersTableBody.addEventListener('click', async e => { if (e.target.classList.contains('btn-toggle-status')) { const orderId = e.target.dataset.id; const newStatus = e.target.dataset.currentStatus === 'pending' ? 'delivered' : 'pending'; try { const response = await fetch(`${API_BASE_URL}/orders/${orderId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': getPassword() }, body: JSON.stringify({ status: newStatus }) }); if (!response.ok) throw new Error('No se pudo actualizar el estado.'); loadInitialData(); } catch (error) { alert(error.message); } } });

    if (getPassword()) { verifyPassword(getPassword()); } else { promptForPassword(); }
});